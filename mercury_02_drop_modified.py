"""
Mercury 02 (drop-modified): laser scheme constructor, project version 1.24 (with python 3.9).

Difference from `mercury_02_copy.py`:
- The split-into-two-parts feature is removed entirely; an experiment is always a single run.
- If there are more submask regions than the port budget can uniquely barcode (i.e. more than
  C(num_ports, concatenations) combinations), the excess submask regions are simply dropped so
  the remaining ones all fit within num_ports fluidic rounds.
"""
...
class App(customtkinter.CTk):
    ...
    def app_exp(self):
        ...
        # drop-modified build: this version never splits the experiment. if there are more
        # submask regions than the number of unique barcodes available for the given port
        # budget (C(num_ports, num_conct)), drop the excess regions so that the remaining ones
        # all fit within num_ports fluidic rounds (bit string length <= num_ports).
        max_barcodes = math.comb(num_ports, num_conct)
        if len(submask_coordinates_um) > max_barcodes:
            dropped = len(submask_coordinates_um) - max_barcodes
            print(f"Warning: {len(submask_coordinates_um)} submasks exceed the {max_barcodes} "
                  f"unique barcodes for {num_ports} ports and {num_conct} concatenations.")
            print(f"Dropping the last {dropped} submask region(s) beyond barcoding capacity.")
            submask_coordinates_um = submask_coordinates_um[:max_barcodes]
            submask_coordinates_px = submask_coordinates_px[:max_barcodes]
        ...
        # single-run experiment: save the full bit scheme and generate one set of cleave maps
