"""
Mercury 02 (split-modified): laser scheme constructor, project version 1.24 (with python 3.9).

Difference from `mercury_02 copy.py`:
- The experiment is ALWAYS separated into two parts, regardless of the number of ports.
- The two parts never share ports: the bit string is divided at its midpoint, so part 1 owns
  the leading ports (e.g. 0-10) and part 2 owns the trailing ports (e.g. 11-20). Part 2 keeps
  its bit strings at full length and pads the part-1 ports with leading zeros so that its 1s
  stay aligned to their original (non-reused) port indices.
"""
...
class App(customtkinter.CTk):
    ...
    def app_exp(self):
        ...
        # this modified build ALWAYS separates the experiment into two parts, and the second part
        # never reuses ports already consumed by the first part. the bit string is divided at its
        # midpoint: part 1 keeps the leading ports (e.g. ports 0-10) and part 2 keeps the trailing
        # ports (e.g. ports 11-20). to keep part 2's ports at their original, non-reused indices,
        # its bit strings retain the full length and are padded with leading zeros for every port
        # that already belongs to part 1.
        split_index = math.ceil(max_index / 2)
        print(f"Experiment folder {self.pth_fld} will always be separated into 2 parts.")
        print(f"Part 1 uses ports 0-{split_index-1}; part 2 uses ports {split_index}-{max_index-1}.")
        # because ports are never reused, the entire experiment has to fit within available ports
        if max_index > num_ports:
            print(f"Warning: bit string length ({max_index}) exceeds {num_ports} available ports.")
            print("Warning: part 2 references ports beyond the physical port count (no reuse).")
        ...
        # split each submask's bit string into two disjoint port ranges
        bit1 = []
        bit2 = []
        for row in fluidic_scheme:
            row1 = row[:7]
            row2 = row[:7]
            # part 1 keeps the leading ports as a compact bit string (ports 0..split_index-1)
            bit_1st_half = row[7][0:split_index]
            # part 2 keeps the trailing ports, but pads the leading (part 1) ports with zeros so
            # that its 1s stay aligned to their original port indices (split_index..max_index-1)
            bit_2nd_half = [0]*split_index + row[7][split_index:]
            # a submask is written into a part only if it is actually cleaved during that part
        ...
        tmp_map = Image.new('P',
                            ...
                            color = (255,255,255))
        # generate cleave maps for each part. the crop/paste box is taken from each row's own
        # stored pixel coordinates (columns w,n,e,s -> indexes[2:6]) instead of indexing the full
        # submask list, so it stays correct even though bit1/bit2 are filtered subsets. part 2's
        # bit strings are full length, so its leading (part 1) ports render as empty cleave maps.
        for folder, bit_rows in ((folder1, bit1), (folder2, bit2)):
            # skip a part with no cleaved submasks (nothing to render)
            if not bit_rows:
                continue
            for i in range(len(bit_rows[0][7])):
                ...
                for indexes in bit_rows:
                    if indexes[7][i] == 1:
                        submask_box = indexes[2:6]
                        this_region = global_mask.crop(submask_box)
                        tmp_map.paste(this_region, submask_box)
                # save this round's cleave map
                tmp_map.save(
                    os.path.join(folder, PARAMS_MAP, ('Round ' + str(i) + '.png')),
                    format = 'PNG'
                )
