...
class Apd(customtkinter.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        self.btn_im2 = customtkinter.CTkButton(
            master = self,
            text = "From Log File",
            command = lambda: self.imp_log(self.ent_pth.get())
        )
    ...
    def imp_fdr(self, dirc):
        ...
        for i, image in enumerate(img):
            self.master.lst_add(PARAMS_CMD[0],[loc[i][1],loc[i][2],loc[i][3]],image,PARAMS_LSR[1])
            self.master.lst_add(PARAMS_CMD[1])
            self.master.lst_add(PARAMS_CMD[2])
    # ---------------------------------------------------------------------------------------------
    def imp_log(self, dirc):
        """
        Function: import scan scheme from experiment datalog.
        """
        loc = pandas.read_csv(os.path.join(dirc, "data.csv")).values.tolist()
        for i in range(0, len(loc)):
            if loc[i][5] == PARAMS_CMD[0].lower():
                self.master.lst_add(
                    PARAMS_CMD[0],
            ...
            elif loc[i][5] == PARAMS_CMD[1].lower():
                self.master.lst_add(
                    PARAMS_CMD[1],
                    loc[i][6],
                    loc[i][7],
            ...
            elif loc[i][5] == PARAMS_CMD[2].lower():
                self.master.lst_add(
                    PARAMS_CMD[2],
                    loc[i][9]
                )
...
class Flu(customtkinter.CTkFrame):
    ...
    def __init__(self, master, item, port=None, rate=None, volm=None, **kwargs):
        ...
        self.lbl_itr = customtkinter.CTkLabel(master=self, width=28, text=f"{item}")
        self.lbl_itr.grid(row=0, column=0, padx=(5,0), pady=5)
        self.lbl_typ = customtkinter.CTkLabel(master=self, width=65, text="Fluidics")
        self.lbl_typ.grid(row=0, column=1, padx=(0,5), pady=5)
        # create input entry boxes
