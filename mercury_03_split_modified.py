"""
Mercury 03 (split-modified): fluid scheme constructor, project version 1.24 (with python 3.9).

Difference from `mercury_03.py`:
- Adds crash/termination resume support. If an experiment stops unexpectedly, simply rerun
  mercury_03 on the same experiment folder and it will pick up where it left off.
- On rerun, the `image_laser` folder is scanned for the last saved per-FOV laser image. Its
  filename encodes the (round, area) it was taken at (same convention mercury_04 uses:
  file[0:4]-1000 = round, file[5:9]-1000 = area). The last saved image marks the resume point.
- That resume point is written to `config_resume_point.csv`. During the LabVIEW run, `update_mask`
  reads it and skips (returns the usual empty "not executed" value) for every (round, area) at or
  before the resume point, so the laser never re-fires on already-completed areas and the
  experiment continues with the rest. The Round CSVs, port list and fov counts are left identical
  to a fresh run, so (round, area) image naming and coordinates stay consistent.
"""
...
def update_mask(img_folder, num_round, area):
    ...
    if num_round < 0 or area < 0:
        ...
        return [[],[],[]]
    # resume support: skip any (round, area) at or before the resume boundary written by
    # app_exp. these areas already have a saved laser image from a previous run, so returning
    # the usual empty "not executed" value keeps the laser from re-firing and lets the
    # experiment continue with the remaining areas. execution order is round-major, area-minor.
